...
public interface StockEmplacementRepository extends JpaRepository<StockEmplacement, Long> {
    ...
    @Query("SELECT se.zone.id, SUM(se.quantite * se.produit.volumeUnitaireM3) FROM StockEmplacement se " +
           "WHERE se.zone IS NOT NULL GROUP BY se.zone.id")
    List<Object[]> sumVolumeGroupedByZone();
}
